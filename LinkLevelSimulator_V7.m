...
function LinkLevelSimulator_V7(InputDopplerShift,MCSIndex,SCS,DMRS,InputChannelModel,NumSubchannels)
close all
...
% PSSCH configuration 
simParameters.PSSCH.SubchannelSize = 12; %!!! ETSI v1113  % Size of a subchannel expressed in PRBs: 10, 12, 15, 20, 25, 50, 75 or 100, see TS 38.331
simParameters.PSSCH.BandwidthMHz = 20;  % Maximum transmission bandwidth in MHz, see Table 5.3.2-1 TS 38.101-1
...
simParameters.PSSCH.AdditionalMCSTable = 1;   % Additional MCS Table indicator, can be set to 0 or 1
...
% Scrambling identifiers (not used)
...
simParameters.SCI1.TimeResourcePSCCH = 3;    % Number of PSCCH symbols in a slot: 2 or 3, see TS 38.331
simParameters.SCI1.FreqResourcePSCCH = 12; %Must be fix %V6 showed 12*NumSubchannel becasue we tested what happen if we have a common setting along the NumSuchannel  % Number of PSCCH PRBs: 10, 12, 15, 20 or 25, see TS 38.331
...
%        simParameters.SNRIn = (-30:1:0);
        ...
        simParameters.SNRIn = (-6:2);
    case 4
        simParameters.SNRIn = (2:10);
    case 6
        simParameters.SNRIn = (8:16);
    case 8
        simParameters.SNRIn = (16:26);
    otherwise
        simParameters.SNRIn = (-15:1:25);
end
...
%    channel.ReceiveAntennaArray.Element = '38.901';
    ...
    switch InputChannelModel
        case 'Highway-LOS'                
%             channel.PathDelays = [0.0 0.0 2.1109e-9 2.9528e-9 17.0328e-9 31.1128e-9 9.1629e-9 10.6761e-9 ...
%                 11.0257e-9 18.5723e-9 19.8875e-9 33.9675e-9 48.0475e-9 25.7370e-9 36.2683e-9 ...
%                 66.7093e-9 139.9695e-9];
%             channel.AveragePathGains = [-0.07 -18.08 -19.9 -13.9 -16.2 -17.9 -14.5 -21.3 -18.7 -14.9 -16.2 -18.5 -20.2 ...
%                 -17.1 -13.8 -28.4 -27.4];
%             channel.AnglesAoA = [-180 -180 -80.2 98.6 98.6 98.6 73.1 -64.3 65.7 -90.9 84.5 84.5 84.5 71.3 ...
%                 -81.5 41.4 -42.6];
%             channel.AnglesAoD = [0.0 0.0 63.4 50.0 50.0 50.0 55.2 -62.6 56.0 53.3 -51.1 -51.1 -51.1 -56.1 58.4 ...
%                  74.7 -71.5];
%             channel.AnglesZoA = [90.0 90.0 75.0 98.4 98.4 98.4 78.1 73.7 105.4 79.1 79.4 79.4 79.4 77.4 80.4 ...
%                  68.1 111.8];
%             channel.AnglesZoD = [90.0 90.0 83.8 86.9 86.9 86.9 85.1 97.3 96.3 85.3 94.4 94.4 94.4 95.5 86.2 81.1 99.4];
%             channel.HasLOSCluster = false; % If false: not caring about the k-factor value for the first cluster
%             channel.KFactorFirstCluster = 0;
...
%             channel.XPR = 9.0;
        ...
        case 'Highway-NLOSv'
%             channel.PathDelays = [0.0 0.0 5.5956e-9 19.6756e-9 33.7556e-9 21.7591e-9 21.8113e-9 27.2207e-9 ...
...
%             channel.AveragePathGains = [-0.2927 -11.8594 -12.5090 -14.7274 -16.4884 -11.8681 -11.3289 -17.8834 -9.9943 -12.7302 -13.9120 ...
%                 -16.8781 -12.9647 -15.1832 -16.9441 -10.7858 -12.3875 -17.3827 -14.7254 -13.5863 -20.9080 ...
%                 -15.5653 -19.7098 -24.7824];
%             channel.AnglesAoA = [-180 -180 -120 -120 -120 -114.6 96.5 77 -124.7 93.1 88.2 80.7 94.4 94.4 94.4 ...
%                 98.1 -99.3 66.8 91.5 -108.4 -89.6 84.4 -81.8 -69.6];
%             channel.AnglesAoD = [0.0 0.0 -52.3 -52.3 -52.3 66.4 -46.7 89.8 -56.8 75.9 85.4 88.9 -46.2 -46.2 -46.2 ...
%                 -50.9 -54.3 88.3 78.0 73.5 -69.7 -62.1 -70.3 -84.5];
%             channel.AnglesZoA = [90.0 90.0 61.5 61.5 61.5 54.8 56.1 34.8 120.7 119.9 48.7 136.9 56.9 56.9 56.9 121.1 121.6 ...
%                 141.6 131.1 51.1 147.1 46.7 32.2 157.3];
%             channel.AnglesZoD = [90.0 90.0 99.2 99.2 99.2 77.2 78.6 106.8 81.2 102.1 103.7 74.0 100.0 100.0 100.0 100.2 77.7 ...
...
%             channel.XPR = 8.0;
        ...
        case 'Urban-LOS'
%             channel.PathDelays = [0.0 0.0 6.4e-9 12.8e-9 11.0793e-9 21.9085e-9 29.6768e-9 36.0768e-9 42.4768e-9 ...
%                 68.4085e-9 82.2944e-9 115.4173e-9 143.2963e-9 146.4136e-9 183.1925e-9 214.1501e-9 326.7825e-9];
%             channel.AveragePathGains = [-0.12 -15.52 -17.7 -19.5 -15.9 -14.6 -9.1 -11.3 -13.1 -19.3 -20.0 -16.3 -17.9 ...
%                 -25.4 -26.9 -22.9 -25.3];
%             channel.AnglesAoA = [-180 -180 -180 -180 -51.8 -51.9 96.8 96.8 96.8 -37.5 -45.7 57.3 -42.1 -17.6 18.6 ...
%                 20.6 -19.1];
%             channel.AnglesAoD = [0.0 0.0 0.0 0.0 93.2 85.4 -49.9 -49.9 -49.9 -97.1 108.5 -90.7 105.5 127.1 127.0 ...
%                 -101.5 125.2];
%             channel.AnglesZoA = [90.0 90.0 90.0 90.0 57.8 119.7 100.0 100.0 100.0 130.3 130.3 58.1 52.5 36.9 145.1 ...
%                 42.8 141.6];
%             channel.AnglesZoD = [90.0 90.0 90.0 90.0 75.1 76.6 84.8 84.8 84.8 107.3 107.7 104.0 107.0 68.4 67.2 ...
%                 69.4 112.0];
%             channel.HasLOSCluster = false; % If false: not caring about the k-factor value for the first cluster
%             %channel.KFactorFirstCluster = 3.48;
%             channel.AngleSpreads = [3.0 17.0 7.0 7.0];
%             channel.XPR = 9.0;
        ...
        case 'Urban-NLOSv'
%             channel.PathDelays = [0.0 0.0 20.1752e-9 34.2552e-9 48.3352e-9 34.3633e-9 37.1866e-9 52.1209e-9 52.7982e-9 ...
...
%             channel.AveragePathGains = [-0.14 -14.93 -8.9 -11.2 -12.9 -17.9 -14.8 -11.9 -10.2 -12.5 -14.2 -11.1 -15.5 -13.8 ...
%                 -12.5 -20.2 -11.7 -19.0 -17.1 -17.5 -18.1 -22.2 -16.4 -19.8];
%             channel.AnglesAoA = [-180 -180 138.4 138.4 138.4 -79.9 -85.1 -100.6 -119.5 -119.5 -119.5 -103.5 92.5 ...
%                 80.7 100.7 -69.4 101.2 69 86.5 91.5 -76.6 -68.1 82.7 -61.8];
%             channel.AnglesAoD = [0.0 0.0 36.0 36.0 36.0 -45.7 60.7 53.6 -34.5 -34.5 -34.5 48.4 -45.8 56.0 55.7 ...
%                 -48.9 51.1 62.7 -43.0 62.4 -50.6 -57.0 -43.1 -50.1];
%             channel.AnglesZoA = [90.0 90.0 81.1 81.1 81.1 118.1 117.3 71.3 103.0 103.0 103.0 108.7 63.7 67.0 ...
%                 109.3 125.9 108.3 58.4 119.8 119.9 120.3 54.1 62.1 56.4];
%             channel.AnglesZoD = [90.0 90.0 84.1 84.1 84.1 74.2 76.4 77.3 97.4 97.4 97.4 99.7 105.6 76.6 76.9 ...
...
% figTx = displayChannel(channel,'LinkEnd','Tx');
% datacursormode on;
% 
% figRx = displayChannel(channel,'LinkEnd','Rx');
% datacursormode on;

% Code to check the maximum Doppler shift values
% velocityValues = (10:10:500);
% for ii = 1:numel(velocityValues)
%     v = velocityValues(ii);
%     fc = 5.9e9;
%     c = physconst('lightspeed'); % speed of light in m/s 
%     fd = (v*1000/3600)/c*fc;
%     fprintf('Velocity %4.1f km/h, MaximumDopplerShift = %8.2f Hz \r\n',v,fd)
% end
...
% LDPC decoder parameters
% Available algorithms: 'Belief propagation', 'Layered belief propagation', 'Normalized min-sum', 'Offset min-sum'
decodeSLSCH.LDPCDecodingAlgorithm = simParameters.PSSCH.LDPCDecodingAlgorithm;   % Check with Baldo
decodeSLSCH.MaximumLDPCIterationCount = simParameters.PSSCH.MaximumLDPCIterationCount;                  % Check with Baldo
...
parfor snrIdx = 1:numel(simParameters.SNRIn) % uncomment for parallel computing
    disp(['SNR:' num2str(simParameters.SNRIn(snrIdx))])
...
%             dmrsLayerSymbols = PSSCH_DMRSsymbols;     
        ...
        % PSSCH Decoding (CONDITIONED on the 1st-stage SCI), soft bits output
       % [TBLLRs, SCI2LLRs, TBrxSymbols, SCI2rxSymbols] = nrPSSCHDecode(PSSCH.Modulation,psschEq,numel(SCI2cw),noiseEst,SCI1.CRCdecRx);
        % PSSCH Decoding (NOT CONDITIONED on the 1st-stage SCI), soft bits output
        ...
        if simLocal.DebugSimulation
            % Plot the data constellations 
%             figure(); hold on; title('TB constellation (RX side)')
%             scatter(real(TBrxSymbols),imag(TBrxSymbols),'sk')
%             scatter(real(psschSymbols),imag(psschSymbols),'sr')
 
%             figure(); hold on; title('2-nd stage SCI constellation (RX side)')
%             scatter(real(SCI2rxSymbols),imag(SCI2rxSymbols),'sb')
%             scatter(real(psschSymbols),imag(psschSymbols),'sr')
        end

%         figure(); hold on;
%         scatter(real(TB_rxSymbols),imag(TB_rxSymbols),'*b')
%         scatter(real(SCI2rxSymbols),imag(SCI2rxSymbols),'or')
        ...
        if simLocal.DebugSimulation
            fprintf('\nSCI1 error? %d, SCI2 error? %d, TB error? %d',SCI1err,SCI2err,TBerr);
        end
...
outFileName = ['Results/V7_SL_BLER_' num2str(simParameters.Carrier.SubcarrierSpacing) 'kHz_' simParameters.PSSCH.Modulation '_' num2str(round(simParameters.PSSCH.TargetCodeRate*1024)) ...
